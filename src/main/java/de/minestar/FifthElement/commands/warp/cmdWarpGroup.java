...
public class cmdWarpGroup extends AbstractExtendedCommand {

    public cmdWarpGroup(String syntax, String arguments, String node) {
    ...
    public void execute(String[] args, Player player) {
        ...
        if (args.length > 0) {
            if (!args[0].matches("([a-zA-Z0-9_])*")) {
                PlayerUtils.sendError(player, Core.NAME, "Groupnames can only contain ['a'-'Z', '0'-'9' and '_']!");
                return;
            }
        ...
        if (args.length < 1) {
            ...
            if (groups == null) {
                PlayerUtils.sendError(player, Core.NAME, "You have no groups!");
                PlayerUtils.sendInfo(player, Core.NAME, "Use '/wgroup <Name> + <Players...>' to create a new group.");
            } else {
            ...
            if (group == null) {
                PlayerUtils.sendError(player, Core.NAME, "Group '" + canonicalGroupName + "' not found!");
                return;
            }
            PlayerUtils.sendMessage(player, ChatColor.GRAY, "-------------------");
            ...
            if (args[1].equalsIgnoreCase("--")) {
                ...
                if (!Core.dbHandler.deleteGroup(group)) {
                    PlayerUtils.sendError(player, Core.NAME, "Could not delete '" + group.getName().replaceFirst(GuestHelper.GROUP_PREFIX, "") + "'!");
                    PlayerUtils.sendInfo(player, Core.NAME, "Please contact an admin.");
                    return;
                }

                // remove from GroupManager & send success
                GroupManager.removeGroup(player.getName(), group.getName());
                PlayerUtils.sendSuccess(player, Core.NAME, "Group '" + group.getName().replaceFirst(GuestHelper.GROUP_PREFIX, "") + "' deleted!");
            } else {
                PlayerUtils.sendError(player, Core.NAME, "Wrong syntax! Usage: " + this.getSyntax());
                PlayerUtils.sendInfo(player, Core.NAME, "Use '/wgroup <Name> + <Players...>' to add players.");
                PlayerUtils.sendInfo(player, Core.NAME, "Use '/wgroup <Name> - <Players...>' to remove players.");
            }
        } else if (args.length > 2) {
            if (args[1].equalsIgnoreCase("-") || args[1].equalsIgnoreCase("+")) {
                ...
                if (args[1].equalsIgnoreCase("-")) {
                    ...
                    if (!GroupManager.hasGroup(player.getName(), internalGroupName)) {
                        ...
                        // create in database
                        if (!Core.dbHandler.addGroup(group)) {
                            PlayerUtils.sendError(player, Core.NAME, "Could not save '" + group.getName().replaceFirst(GuestHelper.GROUP_PREFIX, "") + "' in database!");
                            PlayerUtils.sendInfo(player, Core.NAME, "Please contact an admin.");
                            return;
                        }
                    } else {
                ...
                // update in database
                if (!Core.dbHandler.updateGroupGuestList(group)) {
                    PlayerUtils.sendError(player, Core.NAME, "Could not update '" + group.getName().replaceFirst(GuestHelper.GROUP_PREFIX, "") + "' in database!");
                    PlayerUtils.sendInfo(player, Core.NAME, "Please contact an admin.");
                    return;
                }
                ...
                PlayerUtils.sendSuccess(player, Core.NAME, "Group '" + group.getName().replaceFirst(GuestHelper.GROUP_PREFIX, "") + "' updated!");
                PlayerUtils.sendInfo(player, Core.NAME, "Players in group: " + playersInGroup);
            } else {
                PlayerUtils.sendError(player, Core.NAME, "Wrong syntax! Usage: " + this.getSyntax());
                PlayerUtils.sendInfo(player, Core.NAME, "Use '/wgroup <Name> + <Players...>' to add players.");
                PlayerUtils.sendInfo(player, Core.NAME, "Use '/wgroup <Name> - <Players...>' to remove players.");
