...
public class cmdTeleportTo extends AbstractExtendedCommand
{
    ...
    public void execute(String[] args, Player player)
    {
        ...
        if (args.length == 1) teleportTo(args, player);
        // TELEPORT A PLAYER TO A SECOND PLAYER
        if (args.length == 2) teleportPlayerToPlayer(args, player);
        // TELEPORT THE COMMAND CALLER TO COORDINATES
        if (args.length > 2) teleportToCoords(args, player);
    }

    private void teleportTo(String[] args, Player player)
    {
        if (!checkSpecialPermission(player, PERMISSION_TELEPORT_TO)) return;
        // GET THE TARGET
    ...
    {
        if (!checkSpecialPermission(player, PERMISSION_TELEPORT_PLAYER_TO_PLAYER)) return;
    ...
    {
        if (!checkSpecialPermission(player, PERMISSION_TELEPORT_TO_COORDS)) return;
        // GET THE COORDINATES
        double x;
        double y;
        double z;
        ...
        else targetWorld = player.getWorld();
